...
void drawFire() {
    // Calculate scaling factors for point size based on window size
    float scaleX = glutGet(GLUT_WINDOW_WIDTH) / 130.0f;
    float scaleY = glutGet(GLUT_WINDOW_HEIGHT) / 80.0f;
    ...
    for (int y = HEIGHT / 2 + 1; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) {
            ...
            // Map x and y to match the coordinate system set by gluOrtho2D
            float mappedX = static_cast<float>(x) * (130.0f / static_cast<float>(WIDTH));
            float mappedY = static_cast<float>(y) * (80.0f / static_cast<float>(HEIGHT));

            // Adjust point size based on window size
            float pointSizeX = 2.0f * scaleX;
            float pointSizeY = 2.0f * scaleY;

            // Adjust color intensity
            float colorIntensity = fire[x][y] / 100.0f;

            glColor3f(colorIntensity, 0.0, 0.0);
            glPointSize(pointSizeX < pointSizeY ? pointSizeX : pointSizeY); // Use the smaller scaling factor
            glBegin(GL_POINTS);
