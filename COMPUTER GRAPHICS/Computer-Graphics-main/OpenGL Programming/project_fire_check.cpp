...
void drawFire() {
    glPointSize(12.0); // Set a smaller point size for sharper pixels
    for (int y = 1; y < 10; ++y) {
        for (int x = 70; x < REGION_WIDTH; ++x) {
            ...
            glBegin(GL_POINTS);
            // glVertex2i(x + buildingPosition[0], y + buildingPosition[1] + buildingHeight);
...
/* void drawFire() {
    ...
    for (int y = 1; y < HEIGHT; ++y) {
        for (int x = 0; x < REGION_WIDTH; ++x) {
            float avg = (fire[x][y - 1] + fire[(x - 1 + REGION_WIDTH) % REGION_WIDTH][y - 1] + fire[(x + 1) % REGION_WIDTH][y - 1]) / 3.0;
            fire[x][y] = (avg + rand() % 20 - 10);
            float fireIntensity = fire[x][y] / 100.0;

            for (int i = 0; i < 4; ++i) { // Draw 4 adjacent points for each pixel
                glColor3f(fireIntensity, 0.0, 0.0);
                glBegin(GL_POINTS);
                // Scale the coordinates based on the window size
                glVertex2f((x * 4 + i) * scaleX, (y * 4 + 70) * scaleY);
                glEnd();
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    //glutInitWindowSize(130, 80); // Set the window's initial width & height
    ...
    // glMatrixMode(GL_PROJECTION);
     // gluOrtho2D(0.0, WIDTH, 0.0, HEIGHT);
    glClearColor(1.0f, 1.0f, 1.0f, 1.0f); // White color (R, G, B, Alpha)
